...
const ContactUs = () => {
    ...
    <div className="flex flex-col items-center justify-center h-screen text-3xl text-center bg-stone-900 font-Italiana">
        <h1 className="text-5xl text-white lg:pt-72">Contact <span className="text-white">Us</span></h1>
            ...
            <form className="flex flex-col mx-auto text-center lg:pt-10"  method="post" onSubmit={sendMail}>
                <div className="flex mb-2 space-x-10">
                    <div className="flex flex-col w-full">
                        <label className="flex text-sm text-white">First Name (required)</label>
                        <input className="w-full p-2 text-lg border-2 border-black border-solid lg:mt-2" 
                            type='text' name='name' id="" onChange={handleChange} value={data.name} placeholder="First name" required/>
                    </div>
                    <div className="flex flex-col w-full">
                        <label className="flex text-sm text-white">Last Name</label>
                        <input className="w-full p-2 text-lg border-2 border-black border-solid lg:mt-2" 
                            type="text" name="lastName" id="" onChange={handleChange} value={data.lastName} placeholder="Last name"></input>
                    </div>
                </div>
                <label className="flex text-white lg:text-sm lg:pt-2">Email (required)</label>
                <input className="flex text-lg border-2 border-black border-solid lg:mt-2 lg:px-[12px] lg:py-[8px]" 
                ...
                <label className="flex text-white lg:text-sm lg:pt-2">Phone Number</label>
                <input className="flex text-lg border-2 border-black border-solid lg:mt-2 lg:px-[12px] lg:py-[8px]" 
                    type='phone' name='phone' id="" onChange={handleChange} value={data.phone} placeholder="+1 (xxx)-xxx-xxxx"
                />
                <label className="flex text-white lg:text-sm lg:pt-2">Message (required)</label>
                <textarea className="flex text-lg border-2 border-black border-solid lg:mt-2" 
                    name='message' id='' cols='30' rows='10' onChange={handleChange} value={data.message} placeholder="Enter message here..." required
                />
                <div className="flex justify-center">
                    <button className="flex items-center justify-center text-lg text-center text-black bg-white rounded-lg hover:bg-black hover:text-white lg:h-8 lg:w-20 lg:mt-5" type='submit'>Send</button>
                </div>
